...
function ReadTodos() {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false); // Shared state for CreateTodo modal
  const [isUpdateModalOpen, setIsUpdateModalOpen] = useState(false); // State for UpdateTodo modal
      ...
      {todos && todos.length > 0 ? (
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 px-10 ">
          {todos.map((todo) => (
            <div
              key={todo._id}
              className="p-6 bg-slate-700 cursor-pointer text-gray-400 shadow-lg rounded-lg hover:shadow-xl transition-shadow relative cursor-pointer"
